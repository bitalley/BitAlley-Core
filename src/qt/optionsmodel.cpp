...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BAC);
    ...
    if (!settings.contains("nAnonymizeBitalleyAmount"))
        settings.setValue("nAnonymizeBitalleyAmount", 1000);

    nAnonymizeBitalleyAmount = settings.value("nAnonymizeBitalleyAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBitalleyAmount"))
        SoftSetArg("-anonymizebitalleyamount", settings.value("nAnonymizeBitalleyAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bitalley.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBitalleyAmount:
            return QVariant(nAnonymizeBitalleyAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBitalleyAmount:
            nAnonymizeBitalleyAmount = value.toInt();
            settings.setValue("nAnonymizeBitalleyAmount", nAnonymizeBitalleyAmount);
            emit anonymizeBitalleyAmountChanged(nAnonymizeBitalleyAmount);
            break;
        case CoinControlFeatures:
