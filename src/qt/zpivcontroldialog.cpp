...
std::set<std::string> ZBitalleyControlDialog::setSelectedMints;
std::set<CMintMeta> ZBitalleyControlDialog::setMints;

ZBitalleyControlDialog::ZBitalleyControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZBitalleyControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZBitalleyControlDialog::~ZBitalleyControlDialog()
{
    delete ui;
}

void ZBitalleyControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBitalleyControlDialog::updateList()
{
...
void ZBitalleyControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBitalleyControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBitalley_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBitalleyControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBitalleyControlDialog::GetSelectedMints()
{
...
void ZBitalleyControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
