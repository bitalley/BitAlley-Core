...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BAC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBAC send
enum ZerocoinSpendStatus {
    ZBAC_SPEND_OKAY = 0,                            // No error
    ZBAC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBAC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBAC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBAC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBAC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBAC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBAC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBAC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBAC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBAC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBAC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBAC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBAC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBAC_SPENT_USED_ZBAC = 14,                      // Coin has already been spend
    ZBAC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBAC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZBitalleyBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZBACOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzBACWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzBACTracker> zpivTracker;
    ...
    void setZWallet(CzBACWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzBACTracker>(new CzBACTracker(strWalletFile));
    }

    CzBACWallet* getZWallet() { return zwalletMain; }
    ...
    void setZBitalleyAutoBackups(bool fEnabled)
    {
    ...
    /** zBAC reset */
    boost::signals2::signal<void()> NotifyzBACReset;
