...
public:
    ...
    bool WriteZBACSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBACSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBACSeed_deprecated(uint256& seed);
    bool EraseZBACSeed();
    bool EraseZBACSeed_deprecated();

    bool WriteZBACCount(const uint32_t& nCount);
    bool ReadZBACCount(uint32_t& nCount);
