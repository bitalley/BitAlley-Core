...
CzBACWallet::CzBACWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBACSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBACSeed_deprecated()) {
                    LogPrintf("%s: Updated zBAC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzBACWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBACCount(nCountLastUsed);
    else if (!walletdb.ReadZBACCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBACWallet::Lock()
{
    seedMaster = 0;
}

void CzBACWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBACWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBAC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBACWallet::LoadMintPoolFromDB()
{
...
void CzBACWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBACWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBACWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzBACWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBAC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzBACWallet::SeedToZBAC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBACWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBACWallet::GenerateDeterministicZBAC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBACWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBACWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
